...
const SUB_PATH = process.env.SUB_PATH || 'zhczhc';       // 订阅sub路径，默认为sub,例如：https://google.com/sub
const UUID = process.env.UUID || '4bc2b277-c852-410a-9711-b37ac2ccd5d3';  // 在不同的平台运行了v1哪吒请修改UUID,否则会覆盖
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'databrick.magik.dpdns.org';           // argo固定隧道域名,留空即使用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiYzI0MDhlOTQwMWU5NjlmYTQ1Y2NhMDJiMjBmMmY5MjgiLCJ0IjoiN2NhODZkMWMtMTBlOS00NDU1LWE2OTQtNWFhZTE0OTkzZGI5IiwicyI6IlkyUXlOek5sTXpRdE16WTFPUzAwT1RNM0xUaG1OR1V0TURrNU1UTmpZV1poTWpZMyJ9';               // argo固定隧道token或json,留空即使用临时隧道
...
const CFPORT = process.env.CFPORT || 8443;                    // 优选域名或优选IP对应端口
const PORT = process.env.PORT || 3000;                       // http订阅端口    
const NAME = process.env.NAME || 'databrick';                         // 节点名称
